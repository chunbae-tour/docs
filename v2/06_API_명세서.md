...
GET `/api/v1/payments/history` (✅USER) — cursor 페이징. content[]{id,orderUid,amount,paymentMethod,status,createdAt,updatedAt}
...
POST `/api/v1/community/posts/companions` (201) — Request: title(max200),content(max5000),placeId,placeName(max100),region?,meetingDate(`@FutureOrPresent`),maxMembers(2~50)
Response: postId,title,content,placeId,placeName,meetingDate,maxMembers,currentMembers,status,writer,createdAt. 에러: 404 PLACE_001, 403 AUTH_024
GET 목록(❌) — region/meetingDate 필터+cursor. content[]{postId,chatRoomId,title,placeName,region?,meetingDate,maxMembers,currentMembers,status,writer,createdAt}
GET 단건(❌) — +chatRoomStatus,content,placeId,updatedAt. 에러: 404 COMMUNITY_001
PUT/DELETE (✅작성자만) — 에러: 404, 403 COMMUNITY_003/004

---

## Community 도메인 — 자유 게시글

POST `/api/v1/community/posts/free` (201) — Request: title(max200),content(max5000),imageUrls?(max10개,각max500)
